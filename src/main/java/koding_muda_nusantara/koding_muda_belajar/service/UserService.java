...
public class UserService {
    ...
    public User register(RegisterRequest request) {
        ...
        if ("lecturer".equalsIgnoreCase(request.getRole())) {
            ...
            // Save dan flush untuk mendapatkan ID
            user = lecturerRepository.saveAndFlush(lecturer);
            ...
            // Save dan flush untuk mendapatkan ID
            user = studentRepository.saveAndFlush(student);
        } else {
            throw new RuntimeException("Role tidak valid");
        }

        // Buat balance untuk user baru (setelah user punya ID)
        Balance balance = new Balance();
        balance.setUser(user);
        balance.setAmount(0.0);
        user.setBalance(balance);

        // Save dengan cascade untuk balance
